...
!--------------------------------------------------------------------------
   subroutine io_openfiles()
      ...
      if(io_hi/=0)  open(io_hi,status=s,access=a, file='vel_history.dat')
      s = 'old'
!      a = 'append'
...
!--------------------------------------------------------------------------
   subroutine io_load_state()
      integer :: e, f, i, rd,omode
      double precision :: d

      if(mpi_rnk==0)      print*, "Loading: ",io_statefile
!      e=nf90mpi_open(io_statefile, nf90_nowrite, f)      

      omode = NF90_NOWRITE + NF90_64BIT_OFFSET
      e=nf90mpi_open(MPI_COMM_WORLD, io_statefile, omode, &
           MPI_INFO_NULL,f)
      ...
      e=nf90mpi_get_att(f,nf90_global,'t', d)
      if(d_time<0d0) tim_t = d
      if(mpi_rnk==0 .and. dabs(tim_t-d)>1d-8)  &
         print*,' t    :',d,' --> ', tim_t

      e=nf90mpi_get_att(f,nf90_global,'Re', d)
      if(mpi_rnk==0 .and. dabs(d_Re-d)>1d-8)  &
         print*,' Re   :',d,' --> ', d_Re
      e=nf90mpi_get_att(f,nf90_global,'alpha', d)
      if(mpi_rnk==0 .and. dabs(d_alpha-d)>1d-8)  &
         print*,' alpha:',d,' --> ', d_alpha
      e=nf90mpi_get_att(f,nf90_global,'gamma', d)
...
!--------------------------------------------------------------------------
   subroutine io_load_mpt(f,nm, a)
      ...
      e=nf90mpi_inq_varid(f,nm, i)
      if(e/=nf90_noerr)  print*, 'Field '//nm//' not found!'
      if(e/=nf90_noerr)  stop 'io_load_coll'
      e=nf90mpi_get_att(f,i, 'K',  K__)
      e=nf90mpi_get_att(f,i, 'MM',  M__)
      e=nf90mpi_get_att(f,i, 'NN',  N__)
      ...
      starts=(/1,1,var_N%pH0+1,1/)
      counts=(/i_K,i_M,var_N%pH1+1,1/)
!      e=nf90_get_var(f,i, a%Re(1:i_K,mm,nn),start=(/1,mm_+1,n+1,1/))
      e=nf90mpi_get_var_all(f, i,nf90_double, starts, a%Re);
      starts=(/1,1,var_N%pH0+1,2/)
      e=nf90mpi_get_var_all(f, i, starts, counts, a%Im);

!      e=nf90_get_var(f,i, a%Im(1:i_K,mm,nn),start=(/1,mm_+1,n+1,2/))
   
     if(mpi_rnk==0) print*, "Loaded field!"
...
!--------------------------------------------------------------------------
   subroutine io_save_state()
      character(4) :: cnum
      integer :: e, ncid,cmode
      integer :: md,nd,kd, ReImd, dims(4)
      integer(kind=MPI_OFFSET_KIND) ::  ds(4) 
      integer :: ss
...
!      if(mpi_rnk==0) then
  ...
  !       e=nf90_create('state'//cnum//'.cdf.dat', nf90_clobber, f)
      e= nf90mpi_create(MPI_COMM_WORLD, 'state'//cnum//'.cdf.dat', cmode, &
           MPI_INFO_NULL, ncid)
         
      e=nf90mpi_put_att(ncid, NF90_DOUBLE, 't', tim_t)
      e=nf90mpi_put_att(ncid, NF90_DOUBLE, 'Re', d_Re)
      e=nf90mpi_put_att(ncid, NF90_DOUBLE, 'alpha', d_alpha)
      e=nf90mpi_put_att(ncid, NF90_DOUBLE, 'gamma', d_gamma)
      
!      e=nf90mpi_def_dim(ncid, "N", i_NN, nd)
!      e=nf90mpi_def_dim(ncid, "Z", gsizes(3), dimids(3))
      ds=(/i_K,i_M,i_NN,2/)
      e=nf90mpi_def_dim(ncid=ncid, name="N", len=ds(3), dimid=dims(3))
      e=nf90mpi_def_dim(ncid, 'M', ds(2), dims(2))
      e=nf90mpi_def_dim(ncid, 'K', ds(1), dims(1))
      e=nf90mpi_def_dim(ncid, 'ReIm',ds(4), dims(4))
!      dims = (/kd,md,nd,ReImd/)
      call io_define_mpt(ncid, 'mptR', dims, ss)
      
      e=nf90mpi_enddef(ncid)

      call io_save_mpt(ncid,ss, vel_c)

      e=nf90mpi_close(ncid)
...
!--------------------------------------------------------------------------
   subroutine io_define_mpt(f,nm,dims, id)
      ...
      integer :: e
      e=nf90mpi_def_var(f, nm, nf90_double, dims, id)
...
!      e=nf90_put_var(f,id,a%Im(1:i_K,0:i_M1,0:i_NN1), start=(/1,1,1,2/))
      e=nf90mpi_put_var_all(f, id, a%Re, starts, counts)
      starts=(/1,1,var_N%pH0+1,2/)
      e=nf90mpi_put_var_all(f, id, a%Im, starts, counts)
