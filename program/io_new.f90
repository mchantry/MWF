...
!--------------------------------------------------------------------------
!  Save course grained state
!--------------------------------------------------------------------------
   subroutine io_save_course()
      character(4) :: cnum
      integer :: e, f
      integer :: md,nd,kd, ReImd, dims(4)
      integer :: ss 
      ...
      if(mpi_rnk==0) then
         print*, ' saving course'//cnum//'  t=', tim_t
         e=nf90_create('course'//cnum//'.cdf.dat', nf90_clobber, f)
         ...
         e=nf90_def_dim(f, 'N', i_CN, nd)
         e=nf90_def_dim(f, 'M', i_CM, md)

         dims = (/kd,md,nd,ReImd/)
         call io_define_crs(f, 'crs', dims, ss)

         e=nf90_enddef(f)
      end if

      call io_save_crs(f,ss, mean_t)

      if(mpi_rnk==0)  &
         e=nf90_close(f)

    end subroutine io_save_course
...
!--------------------------------------------------------------------------
   ...
   subroutine io_define_crs(f,nm,dims, id)
      integer,      intent(in) :: f, dims(4)
      character(*), intent(in) :: nm
      integer, intent(out) :: id
      integer :: e
      e=nf90_def_var(f, nm, nf90_double, dims, id)
      e=nf90_put_att(f, id,  'K', i_K)
      e=nf90_put_att(f, id,  'CM', i_CM)
      e=nf90_put_att(f, id,  'CN', i_CN)
    end subroutine io_define_crs
...
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   subroutine io_gather_crs(f,id,a)
      integer,     intent(in) :: f, id
      type (mpt), intent(in) :: a
      integer :: e
      
#ifndef _MPI
      e=nf90_put_var(f,id,a%Re(0:i_DM:i_M1,0:i_DN:i_NN1), start=(/1,1/))
#else
      integer :: r, pN0,pN1

      if(mpi_rnk==0) then
         e=nf90_put_var(f,id,a%Re(0:i_CN1,0:i_DN:var_N%pH1), start=(/1,1,1,2/))         
         do r = 1, mpi_sze-1
            pN0 = var_N%pH0_(r)
            pN1 = var_N%pH1_(r)
            mpi_tg = r
            ...
            call mpi_recv( c1%Im(1,0,0), i_M*(pN1+1)*i_K, mpi_double_precision,  &
               r, mpi_tg, mpi_comm_world, mpi_st, mpi_er)
            e=nf90_put_var(f,id,c1%Re(1:i_K,0:i_DM:i_M1,0:i_DN:pN1), start=(/1,1,pN0/i_DN+1,1/))
            e=nf90_put_var(f,id,c1%Im(1:i_K,0:i_DM:i_M1,0:i_DN:pN1), start=(/1,1,pN0/i_DN+1,2/))
         end do
      else
         mpi_tg = mpi_rnk
         call mpi_send( a%Re(1,0,0), i_M*(var_N%pH1+1)*i_K, mpi_double_precision,  &
