...
!-------------------------------------------------------------------------
   subroutine var_mpt2spec(mp,s)
     ...
     do k=0,i_K0-1
        ...
        k3=k+2*i_K0
        if (m==0) then
           s%Re(k1,m,n)= mp%Re(k1,m,n)
           s%Im(k1,m,n)= mp%Im(k1,m,n) 
           s%Re(k2,m,n)=  ad_n1(nn)*mp%Im(k2,m,n)
           s%Im(k2,m,n)= -ad_n1(nn)*mp%Re(k2,m,n) 
           s%Re(k3,m,n)=  ad_k1(k)*mp%Re(k2,m,n)
           s%Im(k3,m,n)=  ad_k1(k)*mp%Im(k2,m,n) 
        else if (nn==0) then
           s%Re(k3,m,n)= mp%Re(k1,m,n)
           s%Im(k3,m,n)= mp%Im(k1,m,n)            
           s%Re(k2,m,n)= -ad_m1(m)*mp%Im(k2,m,n)
           s%Im(k2,m,n)=  ad_m1(m)*mp%Re(k2,m,n) 
           s%Re(k1,m,n)= -ad_k1(k)*mp%Re(k2,m,n)
           s%Im(k1,m,n)= -ad_k1(k)*mp%Im(k2,m,n) 
        else
   ...
   subroutine var_spec2mpt(s,mp)
     ...
     do k=0,i_K0-1
        ...
        k3=k+2*i_K0
        if (m==0) then
           mp%Re(k2,m,n) = - s%Im(k2,m,n)*ad_n1(nn) &
                + s%Re(k3,m,n)*ad_k1(k)
           mp%Im(k2,m,n) =  s%Re(k2,m,n)*ad_n1(nn) &
                + s%Im(k3,m,n)*ad_k1(k)
           mp%Re(k1,m,n) = - s%Re(k1,m,n)
           mp%Im(k1,m,n) = - s%Im(k1,m,n)
        else if (nn==0) then
           mp%Re(k2,m,n) =  s%Im(k2,m,n)*ad_m1(m) &
                - s%Re(k1,m,n)*ad_k1(k)
           mp%Im(k2,m,n) = -s%Re(k2,m,n)*ad_m1(m) &
                - s%Im(k1,m,n)*ad_k1(k)
           mp%Re(k1,m,n) = - s%Re(k3,m,n)
           mp%Im(k1,m,n) = - s%Im(k3,m,n)
        else
