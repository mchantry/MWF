...
!   implicit none
   ...
   complex(C_DOUBLE_COMPLEX), pointer, private :: state_inp(:,:),state_res(:,:)!(i_KK,0:i_3M-1),state_res(i_KK,0:i_3M-1)
   complex(C_DOUBLE_COMPLEX), pointer, private :: state_mid(:,:)!(i_KK,0:i_Na)
   real(C_DOUBLE), pointer,            private :: state_phy(:,:)!(i_KK,0:i_3N-1)
...
!------------------------------------------------------------------------
   subroutine tra_precompute
      ...
      type(C_PTR) :: pi,pr,pm,pp
      
      pi = fftw_alloc_complex(int(i_KK * i_3M, C_SIZE_T))
      call c_f_pointer(pi, state_inp, [i_KK,i_3M])
      pr = fftw_alloc_complex(int(i_KK * i_3M, C_SIZE_T))
      call c_f_pointer(pr, state_res, [i_KK,i_3M])
      pm = fftw_alloc_complex(int(i_KK * (i_Na+1), C_SIZE_T))
      call c_f_pointer(pi, state_mid, [i_KK,i_Na+1])
      pp = fftw_alloc_real(int(i_KK*i_3N, C_SIZE_T))
      call c_f_pointer(pp, state_phy, [i_KK,i_3N])
   ...
   subroutine tra_spec2phys(s, p)
      ...
      do n = 0,var_N%pH1
         ...
         do m = -(i_M-i_MM),i_MM1
            mm=modulo(m,i_3M)
            ms=modulo(m,i_M)
            state_inp(:,mm+1)=dcmplx(s%Re(:,ms,n),s%Im(:,ms,n))
         end do
      ...
      call tra_T2Ts()
      do m = 0,var_M%pH1
         state_mid(:,i_NN+1:)=0d0
         state_mid(:,1:i_NN)=Ts(:,:,m)
   ...
   subroutine tra_spectest(s)
      ...
      do n = 0,var_N%pH1
         ...
         do m = - (i_M - i_MM),i_MM1
            mm = modulo(m,i_3M)
            ms = modulo(m,i_M)
            s%Re(:,ms,n)= dble(state_inp(:,mm+1))
            s%Im(:,ms,n)=dimag(state_inp(:,mm+1))
         end do
      end do
